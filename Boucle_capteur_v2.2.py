...
def lecture_capteurs(listeCapteurs, tabCouleurs, i2c):
    ...
    return tabCouleurs
def recuperer_exercices(niveau): # attention trouver comment obtenir un résultat diférents à chaque fois
    cnx = sqlite3.connect('DVMaths.sqlite' )

    cursor = cnx.cursor()
    recup_exo = "SELECT  calcul   FROM ExercicesDeCalcul Where  niveau = ? "
    data = (niveau)
    cursor.execute(recup_exo, data)#execution d'une requête SQL a l'aide de la methode execute()
    result = cursor.fetchall()
    return result[0]
def recuperer_resultat(niveau):

    cnx = sqlite3.connect('DVMaths.sqlite') #creation d'un objet-connection a l'aide de la methode connect()
    cursor = cnx.cursor() #creation d'un objet-interface a l'aide de la methode connect()
    recup_result = "SELECT  Resultat   FROM ExercicesDeCalcul Where  niveau = ?  "
    data = (niveau)
...
# envoie du calcul à la fin
                    insert_calcul(id, calculDonne, calculPose, resultatAbsolu, niveau)
...
# envoie du resultat posé
                        insert_resultat(id, calculDonne, calculPose, resultatAbsolu, resultatPose, niveau)
                        break
                    else:
...
# si btn2 demande exo bdd
                ...
                print("demande exo")
                calculDonne = recuperer_exercices(niveau)
                print(calculDonne)
                resultatAbsolu = recuperer_resultat(niveau)
